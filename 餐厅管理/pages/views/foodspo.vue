				...
				<view class="unicorn-margin-top-common">			
					<view class="unicorn-card">
						...
						<view class="unicorn-card-context">
							<plugin-table :tableData='fpoTableData' :columns='fpoColumns' :showBottomSum='true' :stickSide='true'>
							</plugin-table>
						</view>
					</view>
				</view>
		...
		methods: {
			...
			refreshData(week) {
				...
				this.weekList.forEach(week => {
					var target = {}
					this.fpoColumns.forEach(fpo => {
						var key = fpo['key']
						target[key] = 0
					})
					target.week = this.convertWeekDay(week)
					target.date = 'N/A'
					this.fpoTableData.push(target)
				})
				http.get('http://81.70.37.99/JRestaurant/weixin/ListFoodsPOByWeek', {params: {week: week}}).then(res => {
					this.fpoList = res.data
					this.loadModal = false
					this.fpoList.forEach(fpo => {
						this.fpoTableData.forEach(row => {
							if (this.convertWeekDay(fpo.WeekDay) === row.week) {
								var key = "Id" + fpo.VendorId
								row[key] = row[key] + parseFloat(fpo.Amount)
								row.date = this.timeformat(fpo.AddDate)
							}
							var total = 0
							for (var key in row) {
								if (key.indexOf("Id")>-1) {
