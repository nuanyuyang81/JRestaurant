	...
	<view>
		<scroll-view scroll-y class="page">
			<cu-custom bgColor="bg-gradual-blue" :isBack="true">
				...
				<block slot="content">酒水营业额</block>
			</cu-custom>
				...
				<view class="unicorn-margin-top-common">			
					<view class="unicorn-card">
						...
						<view class="unicorn-card-context">
							<plugin-table :tableData='dtTableData' :columns='dtColumns' :showBottomSum='true' :stickSide='true'>
							</plugin-table>
						</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			this.loadModal = true
			http.get('http://81.70.37.99/JRestaurant/weixin/ListTurnOverTypeByAim', {params: {aim: 1}}).then(res => {
				var list = res.data
				this.dtColumns.push({
						"title": "总额",
						"key": "total",
			...
			refreshData(week) {
				...
				http.get('http://81.70.37.99/JRestaurant/weixin/ListDrinksTurnOverByWeek', {params: {week: week}}).then(res => {
					...
					this.dtList.forEach(dt => {
						this.dtTableData.forEach(row => {
							if (this.convertWeekDay(dt.WeekDay) === row.week) {
								var key = "Id" + dt.TypeId
								row[key] = row[key] + parseFloat(dt.Amount)
								row.date = this.timeformat(dt.AddDate)
							}
							var total = 0
							for (var key in row) {
								if (key.indexOf("Id")>-1) {
